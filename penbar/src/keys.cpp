...
// ОЖИДАНИЕ УХОДА ПЕРА УБРАНО, и вот почему. Раньше кнопка мыши, нажатая
// пером, не срабатывала сразу, а ждала, пока указатель уйдёт с панели: тогда
// нажатие приходилось под перо, уже на холсте. На планшете это не работает:
// когда перо отрывают от экрана, указатель ОСТАЁТСЯ там, где был, то есть на
// панели, — и ждать можно вечно. Со стороны это «залипающая кнопка не
// нажимается вовсе». А ради чего ждали, теперь делают площадки: они держат
// свою кнопку сами, пока по ним ведут. Нажатие уходит сразу и приходится
// в якорь — ту же точку, куда жмёт площадка, и она у неё работает.
