...
async function useOwnPhotos(list) {
  const all = [...list];
  // CineFlow: video intake is ON. Upstream keeps it off until its frame
  // selection meets their quality bar; for us the video IS the input —
  // an Insta360 walk-through exported as a 360 mp4. A video with fewer
  // than two photos beside it goes down the sharp-frames path.
...
async function useOwnVideo(file) {
  ...
  try {
    const { frames, duration } = await extractSharpFrames(file, {
      // CineFlow: 8K 360 frames are downscaled at capture (see video.js) —
      // the pano slicer never reads more than 5120 px of width anyway
      maxW: 5120,
