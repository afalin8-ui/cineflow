 ...
 * @property {number} [maxW]               cap captured frame width (CineFlow: 360 video)
...
export async function extractSharpFrames(file, opts = {}) {
  ...
  try {
    ...
    // CineFlow: opts.maxW caps the captured width. An 8K 360 video gives
    // 7680-px frames, ~6 MB of JPEG each — 140 of them is close to a
    // gigabyte held in a tablet tab, while the cube faces sliced from a
    // pano are clamped at 1280 px anyway (4 × 1280 = 5120 px of pano is
    // all the slicer ever reads). Downscaling at capture costs nothing.
    const cw = opts.maxW && vw > opts.maxW ? opts.maxW : vw;
    const ch = Math.max(2, Math.round(vh * cw / vw));
    if (cw !== vw) log(`capture downscaled to ${cw}x${ch} (maxW ${opts.maxW})`);
    const capCv = mkCanvas(cw, ch);
