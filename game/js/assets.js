...
function prepare(root, cfg) {
  ...
  root.traverse(o => {
    ...
    if (o.geometry) {
      ...
      // Без нормалей модель освещается как плоская заливка
    ...
    if (o.material) {
      ...
      for (const m of mats) {
        ...
        /* Двусторонний рендер — не прихоть, а страховка. Скачанная
           или сгенерированная модель почти никогда не бывает
           герметичной: после упрощения сетки в корпусе остаются
           открытые края, а у части полигонов нормали смотрят внутрь.
           При односторонней отрисовке и то и другое выглядит дырами,
           сквозь которые видно космос. Лишних вызовов отрисовки это
           не добавляет — просто не отсекаются задние грани. */
        m.side = THREE.DoubleSide;
        /* Генераторы любят отдавать зеркало: metalness 1 и roughness 0.
           В сцене с картой окружения такой корпус превращается в каплю
        ...
        if (m.isMeshStandardMaterial) {
          ...
          /* Карту окружения приезжей модели даём вполсилы. Свои
             корпуса рисовались под нашу туманность, а чужой материал
             почти всегда «металл в ноль шероховатости»: на полной
             интенсивности он превращается в каплю хрома, отражающую
             весь фон. Сами карты автора при этом не трогаем. */
