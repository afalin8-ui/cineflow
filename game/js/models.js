...
export function buildStrike(role, faction) {
  ...
  /* Истребитель в масштабе линкора — точка в один пиксель. В Empire
     at War и Homeworld авиацию сознательно рисуют крупнее правды:
     иначе бой превращается в мерцание, а игрок не понимает, кто там
     дерётся. Втрое крупнее «честного» — компромисс, при котором
     звено читается, но не спорит с корветом. */
...
void main() {
  ...
  /* Закон Бугера: свет гаснет в толще воды экспоненциально, поэтому
     видимость дна зависит от глубины, а не от расстояния до берега.
     Это ключ ко всему: пока слой тонкий, сквозь него видно грунт, и
     отмель читается сама собой. Если вместо этого залить мелководье
     непрозрачной бирюзой — вокруг каждого озера появляется яркая
     кайма, будто его обвели кистью. Так и было. */
  float k = 1.0 - exp(-depth * 5.0);       // 0 у берега, ~1 на глубине

  vec3 col = mix(uShallow, uDeep, smoothstep(0.05, 0.55, depth));
  /* По касательной вода отражает небо — именно это делает её
     «мокрой». На отмели отражение приглушено: сквозь тонкий слой
     видно дно, и небу там взяться неоткуда. */
  col = mix(col, uSky, fres * 0.5 * (0.25 + 0.75 * k));

  /* Рябь должна читаться и там, где солнечной дорожки нет: склон
     волны, повёрнутый к светилу, чуть светлее обратного. Дорожка
     бывает в одном месте кадра, а рябь нужна по всей воде — без неё
     озеро выглядит налитым лаком. */
  col *= 1.0 + (n.x * uSun.x + n.z * uSun.z) * 0.55;
  ...
  // Держим искры ниже порога свечения: всё, что ярче, bloom
  // размазывает в белые кляксы по всему озеру
  col += vec3(1.0, 0.95, 0.86) * (wide * 0.40 + glint * 2.2);

  // Пена узкой полосой у самой кромки, дышит вместе с волной.
  // Озеро — не прибой: полоса еле заметна, иначе она сама и станет
  // той самой белой каймой
  float edge = 1.0 - smoothstep(0.0, 0.018, depth);
  float foam = edge * (0.45 + 0.55 * sin(uTime * 1.8 + vW.x * 0.5 + vW.z * 0.4));
  col = mix(col, vec3(0.80, 0.88, 0.92), foam * 0.18);

  /* Прозрачность идёт по той же экспоненте: у кромки почти стекло,
     на глубине — плотная толща. Раньше вода становилась глухой уже
     на четверти глубины, и дно пропадало там, где его как раз
     и должно быть видно. */
  float alpha = clamp(0.05 + k * 0.86 + fres * 0.22 + foam * 0.15, 0.0, 0.94);
