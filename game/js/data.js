...
export const HYPER = {
  ...
  /* Флот приходит в бой не разом. Гиперпереход не выстраивает
     эскадру в линию: корабли вываливаются вразнобой, и часть
     доходит позже. Играется это так, что первый залп принимает
     авангард, а исход решает, успеет ли подойти остальное. */
  firstWave: 0.6,         // доля флота, что выходит из гипера сразу

  /* Дальний гипер. Можно дёрнуть корабли из СВОИХ систем в
     нескольких прыжках от боя — но лететь им долго, и пока они
     летят, их родная система стоит пустая. */
  farMaxHops: 3,          // дальше трёх прыжков не дозовёшься
  farBaseDelay: 45,       // секунд до прибытия из соседней системы
  farPerHop: 38,          // и столько за каждый следующий прыжок
};
...
export const GALAXY_MAP = {
  systems: [
    ...
    { id: 'atropos',    name: 'Атропос',      pos: [2, 14, -24],  income: 160, slots: 3, biome: 'ice',    traits: ['lowg'], rings: true },
    ...
    { id: 'phlegethon', name: 'Флегетон',     pos: [-46, 8, 30],  income: 175, slots: 4, biome: 'desert', traits: ['dust'], rings: true },
    ...
    { id: 'morbelt',    name: 'Пояс Мора',    pos: [-60, -14, 58],income: 250, slots: 4, biome: 'rock',   start: 'plektor', traits: ['scrapyard', 'rich'], rings: true },
...
export const PLANET_BUILDINGS = {
  ...
  warpgate: { id: 'warpgate', name: 'Гиперпространственные врата', cost: 1300,
              desc: 'Без них лёгкие корабли не могут уйти из системы' },
...
/* ─────────────────────────────────────────────────────────────
   ...
   Четыре клана строятся принципиально по-разному — это и есть
   главное различие в наземном бою, важнее любых цифр в характеристиках.

   Тройден  — базы не строит: готовые модули падают с орбиты в любую
              точку карты. Сборщиков нет, привязки к штабу нет,
              зато каждый модуль дороже.
   Плэктор  — тяжёлая индустрия: строит только рядом с уже стоящим,
              зато дёшево и быстро. База расползается пятном.
   Девиан   — модульная крепость: пристраивается ВПЛОТНУЮ к штабу,
              дальше определённого радиуса не строит вовсе. Компактно
              и живуче, но развернуться негде.
   Рииз     — рой дронов: строит где угодно без привязки, но каждая
              постройка дольше — нанорой собирает её из ничего.
   ───────────────────────────────────────────────────────────── */

export const BASE_DOCTRINE = {
  troyden: {
...
export const doctrineOf = f => BASE_DOCTRINE[f] || BASE_DOCTRINE.plektor;

/* ─────────────────────────────────────────────────────────────
   УРОВНИ СЛОЖНОСТИ.

   Крутим не «меткость врага» в одиночку, а четыре ручки сразу,
   потому что сложность в стратегии — это темп, а не цифры урона:

   eco    — сколько ИИ добывает на земле (множитель к его добыче)
   tempo  — как часто он принимает решения (меньше — чаще)
   wave   — сколько войск копит перед атакой (меньше — давит раньше)
   aim    — множитель урона его оружия
   camp   — доход и наука ИИ на карте системы
...
export const DIFF_IDS = ['easy', 'normal', 'hard', 'brutal'];
export const diffOf = id => DIFFICULTY[id] || DIFFICULTY.normal;

/* ─────────────────────────────────────────────────────────────
   ТАКТИКИ ПОВЕДЕНИЯ.

   Одна и та же рота ведёт себя по-разному, и это решает игрок,
   а не автор кода:

   Охрана   — держит участок. Ввяжется в бой с тем, кто подошёл,
              погонится недалеко и ВЕРНЁТСЯ на место. Годится
              для прикрытия базы и сборщиков.
   Оборона  — не двигается вовсе, только стреляет. Артиллерия,
              ЗСУ и всё, что должно стоять там, где поставили.
   Нападение — идёт на всё, что видит, и преследует до конца.
              Годится для зачистки, но так и уводят отряд
              по частям.

   `see` — во сколько раз шире обычного юнит ищет цели,
   `leash` — насколько далеко отпускает свою точку (0 — никуда).
   ───────────────────────────────────────────────────────────── */

export const STANCES = {
  guard:  { id: 'guard',  name: 'Охрана', short: 'Охрана', see: 1, leash: 60,
            hint: 'Держит участок: подерётся с тем, кто подошёл, и вернётся на место' },
  hold:   { id: 'hold',   name: 'Оборона', short: 'Оборона', see: 0.75, leash: 0,
            hint: 'Стоит намертво и работает только по тому, до кого дотягивается' },
  hunt:   { id: 'hunt',   name: 'Нападение', short: 'Атака', see: 2.2, leash: 1e9,
            hint: 'Идёт на всё, что видит, и преследует до конца' },
};
...
/* ─────────────────────────────────────────────────────────────
   ...
   Собственный гипердвигатель есть только у больших кораблей —
   флагманов и носителей. Всё остальное (корветы, фрегаты, РЭБ,
   крейсеры) прыгает через стационарные врата и без них из системы
   не уйдёт.

   Отсюда и берётся карта фронтов: система без врат — тупик, из
   которого москитный флот не вытащить, а система с вратами —
   узел, за который дерутся. И отсюда же смысл корабля-взломщика:
   он не топит флот, он его запирает.
   ───────────────────────────────────────────────────────────── */

export const HYPER_OWN = ['capital', 'carrier'];   // классы со своим приводом

export function shipHasDrive(def) {
  return !!def && (HYPER_OWN.includes(def.cls) || !!def.hangar);
}

export const BREAKER = {
  id: 'breaker', name: 'Корабль-взломщик', short: 'ВЗЛ', cost: 1900,
  jamTurns: 3,
  desc: 'Уходит к чужой системе и глушит её гиперворота на три хода: ' +
        'подкрепления туда не придут и оттуда не уйдут. Расходуется ' +
        'при использовании — обратно не возвращается.',
};
...
/* ─────────────────────────────────────────────────────────────
   ...
   Обратная сторона «Давления Земли»: то, что уцелело в космосе,
   даёт способности на земле. Именно это связывает слои — выиграть
   орбиту становится выгодно не только ради права высадиться.

   Каждая способность привязана к КЛАССУ уцелевшего корабля. Погиб
   последний носитель — кнопка десанта гаснет и покрывается помехами.
   ───────────────────────────────────────────────────────────── */

export const ORBITAL_SUPPORT = {
  strike: {
    id: 'strike', name: 'Орбитальный удар', short: 'УДАР',
    from: 'capital',          // нужен живой флагман или крейсер
    cooldown: 70, radius: 26, dmg: 1400, delay: 2.2,
    desc: 'Главный калибр с орбиты бьёт по указанной точке. Сносит ' +
          'постройку с одного попадания, но целится две секунды — ' +
          'по технике на ходу почти не попасть.',
  },
  drop: {
    id: 'drop', name: 'Орбитальный десант', short: 'ДЕСАНТ',
    from: 'carrier',          // нужен живой носитель
    cooldown: 95, unit: 'tank', count: 3, delay: 3,
    desc: 'Носитель сбрасывает звено тяжёлой техники прямо в бой. ' +
          'Капсулы падают там, где укажешь, — можно и в тыл врагу.',
  },
  emp: {
    id: 'emp', name: 'ЭМИ-удар', short: 'ЭМИ',
    from: 'ecm',              // нужен живой корабль РЭБ
    cooldown: 80, radius: 90, disable: 15, delay: 1.5,
    desc: 'Обесточивает вражеские постройки в радиусе: пятнадцать ' +
          'секунд они не стреляют и ничего не производят.',
  },
};

/* ── ТОТАЛЬНАЯ БОМБАРДИРОВКА (ЭКСТЕРМИНАТУС).

   Способность последней надежды: весь флот на орбите бьёт по одному
   квадрату разом. Наносит фатальный урон по площади — и своим тоже,
   потому что с орбиты не разбирают, кто где. Меняет ландшафт:
   выжженная земля замедляет технику.

   Цена платится не в бою, а на карте: планета получает статус
   «экологическая катастрофа» и после захвата приносит гроши. Это и
   делает Экстерминатус решением, а не кнопкой «победить». */

export const EXTERMINATUS = {
  name: 'Тотальная бомбардировка', short: 'ЭКСТ',
  needFleet: 3,        // меньше трёх кораблей на орбите — залпа не выйдет
  delay: 6,            // столько секунд от приказа до накрытия
  radius: 210,         // четверть карты: это удар по площади, а не точечный
  dmg: 26000,          // сносит штаб целиком, не оставляя выживших
  volleys: 14,         // залп идёт очередью почти семь секунд
  burnSlow: 0.45,      // техника на выжженной земле ползёт вдвое медленнее
  burnFor: 90,         // и так почти полторы минуты
  ecoPenalty: 0.15,    // доход планеты после такого — шестая часть
  desc: 'Весь флот бьёт по указанному квадрату почти семь секунд ' +
        'подряд. В радиусе не остаётся ничего — ни чужого, ни своего. ' +
        'Выжженная земля надолго вязнет под гусеницами, а планета ' +
        'приносит шестую часть дохода до конца кампании.',
};

/* Какие способности открыты флотом на орбите. На вход — список
   {id, count} уцелевших кораблей и клан, на выход — набор id. */
export function supportFrom(faction, fleet) {
  const have = new Set();
  for (const item of fleet || []) {
    const d = (SHIPS[faction] || []).find(x => x.id === item.id);
    if (!d || !item.count) continue;
    if (d.hangar) have.add('carrier');
    else if (d.ecm) have.add('ecm');
    else if (d.cls === 'capital') have.add('capital');
  }
  return Object.values(ORBITAL_SUPPORT)
    .filter(a => have.has(a.from))
    .map(a => a.id);
}
